...
- **Two mutually-exclusive global modes govern the pad grid — loop mode and instrument mode, both off by default.** Turning one on turns the other off (enforced in the reducer): they each change what the grid means as a whole, so having both on at once would be ambiguous.
  - **Loop mode off** (the baseline, regardless of instrument mode): a pad always plays — a quick tap plays the sample through in full, holding past ~200ms gates it (the sound follows your finger, stopping the instant you release). A quick tap's behavior is unchanged from before gating existed; gating only kicks in on a deliberate hold.
  - **Loop mode on** (its own button in the floating FAB cluster — see Layout): tapping a pad toggles its loop on or off directly instead of playing a one-shot; gating doesn't apply there, since a loop toggle is discrete. Multiple pads loop together freely (see Sync below). This replaced an even earlier design where a pad had its own dedicated Loop button in the action bar.
  - **Instrument mode on** (its own FAB, next to loop mode's): turning it on always prompts "choose an instrument" first — there's no implicit "whatever was there before." Picking one lays its keys across the visible pads (confirmed first if any pad already has a sound). Pads keep playing exactly like the loop-off baseline (tap/gate) — the point is playing the instrument by hand, not a new pad-tap behavior — but every pad holding one of an instrument's keys shows a small badge (its icon) so it's visually obvious the grid is a keyboard right now, independent of whether the mode toggle itself is on. While instrument mode is on, holding the record FAB captures the series of pad presses during the hold as a performance instead of recording from the microphone — bounced offline (`OfflineAudioContext`, replaying each hit's trim/effects at its captured timing — see `engine/bounce.ts`) into a single new sample the instant you release, then reviewed exactly like a mic recording (name it, assign to a pad, or keep in the library only).
...
- **Floating record + metronome + loop-mode + instrument-mode cluster** (fixed, global on every page except while the pad edit popup is open, bottom-right): press-and-hold the record button to record — the hold itself *is* the recording gesture, release stops it, however long that was. Recording was never tied to one screen or page; a name/waveform-preview/assign-or-discard review appears as a modal overlay regardless of where you started recording from, so it's never blocked by which page happens to be open. A smaller metronome toggle, a loop-mode toggle, and an instrument-mode toggle sit beside it, in thumb reach — grouped together because all four are global utilities independent of transport state, unlike everything in the PlayBar. Loop mode and instrument mode are the two hidden while the pad edit popup is open: that popup has its own pad-specific loop button (see below), so the global toggles would sit right next to it doing something confusingly different.
- **Pads page (home)**: the pad grid is the hero content, full width, nothing sharing space with it, laid out 3-per-row so each pad is a generous ~100×100px tap target (default pad count 9 fills this to a clean 3×3). The pad itself is a single undivided tap-to-play/tap-to-loop zone (see Pad Playback Behavior above) showing only live "playing"/"looping"/"muted" visual feedback and, while it holds an instrument key, a small badge for that instrument — no nested controls. Tapping a pad selects it and surfaces a summary bar below the grid with four full-sized actions in a plain 2×2 grid, in reading order: **Mute**, **Effects**, **Edit →**, **Library**. Loop used to live here too; it's now driven by the global loop-mode toggle instead, so a dedicated per-pad button would be redundant. Library opens a popup listing every library sample so you can drop one straight onto the selected pad — the reverse direction of the Library page's own "Assign…" action, which starts from a sample and asks which pad. Moving these off the pad face and into generously-sized buttons fixed a real touch-precision problem: a small icon crowded into the corner of an already-small tile was hard to hit reliably.
...
- **Library page**: instruments (build from a preset or a recording, delete) sit above the raw sample list (rename, reorder, delete, assign, waveform thumbnails) — both get their own page, tucked out of the way of the pads, for occasional housekeeping rather than competing with the main pad-focused screen. Applying an instrument to the grid lives on the Pads page instead (Instrument Mode), not here.
...
interface Transport {
  ...
  padInstrumentModeEnabled: boolean;    // global: mutually exclusive with padLoopModeEnabled
}
