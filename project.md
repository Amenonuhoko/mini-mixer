...
- **Loop button: starts or stops a loop directly, in one tap — but lives in the selected-pad action bar below the grid, not on the pad itself.** It was originally a small icon nested in the corner of the pad tile; in practice that put a ~28px touch target right up against the pad's own large tap-to-play zone, and loop specifically gets tapped rhythmically during live play, making mis-taps costly. The pad face is now a single undivided tap target — always plays a one-shot — with purely informational visual feedback (glow, pulse, equalizer bars) that needs no touch precision. Tapping a pad selects it and surfaces a 3-button action row (Loop, Mute, Edit →) sized for reliable touch (~103×57px each). Loop's on/off look is still driven directly by whether the pad is actually looping right now, not a stored preference — there's nothing else it could mean. (This replaced an earlier design where a pad had a persisted `loop` mode that changed what tapping the pad body did — confusing in practice, since you had to remember which mode a pad was in. Looping is now purely a live, momentary engine state, not app data.)
...
- Audio effect dials: pitch, speed, filter, volume, grit, echo — six, per-pad (each pad has its own settings, independent of which library sample it currently references).
...
- **Bottom PlayBar** (fixed, global on every page): play/pause, BPM, loop-once-vs-continuous, metronome. Shapes *how* playback sounds; deliberately kept separate from the top nav's utility actions, which are about getting around and emergency control instead. The metronome's own button starts and stops it directly — it's not slaved to play/pause, so it can be used on its own to find a tempo before the sequencer is even running.
...
- **Pads page (home)**: the pad grid is the hero content, full width, nothing sharing space with it, laid out 3-per-row so each pad is a generous ~100×100px tap target. The pad itself is a single undivided tap-to-play zone showing only live "playing"/"looping"/"muted" visual feedback — no nested controls. Tapping a pad selects it and surfaces a summary bar below the grid with three full-sized actions: **Loop**, **Mute**, and **Edit →** (the only way into the next page). Moving loop/mute off the pad face and into generously-sized buttons fixed a real touch-precision problem: a small icon crowded into the corner of an already-small tile was hard to hit reliably, especially loop, which gets tapped rhythmically during play.
- **Pad edit page** (reached via "Edit Sound," not a nav tab — a drill-down detail view, not a top-level destination): the per-pad dials (pitch/speed/filter/volume/grit/echo) and the trim editor get the entire page. A back button returns to Pads. Shows the pad's live playing/looping state at the top, since you're often adjusting a sound while it's audible.
