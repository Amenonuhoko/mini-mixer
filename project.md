...
- **Song arrangement** (a collapsible panel above the sequencer): create or duplicate patterns, rename and select the one being edited, then arrange named sections in any order. Each section links to a pattern and repeats it 1–32 times; sections can be renamed, reassigned, copied, moved, or removed. Pattern and Song buttons choose the transport's playback scope. Song playback follows the complete ordered timeline, respects Play once / Loop, highlights the current section, and can be rendered into one sample with **Save song** or downloaded with **Export WAV**. The arrangement and playback scope are included in autosave and project files; older projects open in Pattern mode.
