...
- **Where presets live** — both places, never modal:
  - **Styles dock** (the ✦ button on the Sequencer, or the "Blank canvas?" prompt on an empty pattern): inline under the sequencer on wide screens; on phones a drawer docked above the tab bar, with the page shrinking to sit above it, so the beat stays visible and playable. On top, **the mix**: each bank's current style and take, an intensity slider, a new take and remove; and the beat's chord names with **New chords** (a new progression from the chords layer's style — every preset layer is rewritten to follow). Below, **the library**: every style card has a one-tap button per layer (`Drm · Bass · Chd · Mel`, lit while that layer is playing — tap again for a new take) and a ✦ for a whole beat.
  - **Per-bank strip**: the pad module shows the active bank's strip (new take, intensity, and a scrolling row of styles — tap one to put its layer in this bank); in the sequencer each bank's head has a style chip that opens the same strip.
...
- **Sequencer module**: header (pattern name, step-count stepper in groups of four, the ✦ Styles dock toggle), one toolbar — entry behavior on the left (**Gate**, **Preview**), pattern actions on the right (**Save**, **Load**, **Hide/Show**, **Clear trace**, **Clear**) — then the grid read as a timeline: beat groups set apart, each group's downbeat a touch brighter, the playhead column lit orange. Each row's number chip glows with that pad's live level and opens the library picker to swap its sound; **✕** removes a row (confirmed); dragging across cells paints a run; **+ Add row** grows the pad count.
- **Song arrangement** (a collapsible panel above the sequencer): start from a Verse/Chorus, Hook-first, or full-pop structure, then change any section. A template creates a distinct pattern for each unique part and links repeated parts to that pattern; existing named patterns are reused. Tap **Edit Chorus** (or another section) to select its pattern and jump straight to the sequencer; **Song** takes you back. Each section shows which banks have programmed hits and can audition only that section or play from there through the ending, so transitions can be judged in context. Sections can be renamed, reassigned, copied, moved, removed, and repeated 1–32 times. Pattern and Song choose the transport's playback scope. Song playback follows the ordered timeline, respects Play once / Loop, highlights the current section, and can be rendered into one sample with **Save song** or downloaded with **Export WAV**. The arrangement and playback scope are included in autosave and project files; older projects open in Pattern mode.
