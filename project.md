...
### Styles & starter beats ("where do I start?")

The answer to the blank canvas after "what to play": pick a style and get a real, editable beat — whole, or one layer at a time.
...
- **Rhythm lines** are one bar of 16ths in a readable notation: `x` always, `o` often (65%), `-` sometimes (25%), `.` never. Every choice with alternatives (progressions, lines) is a list. A one-bar line is rolled once and repeated so the groove is steady; the last bar is the style's fill, or a fresh roll — a variation on the turnaround.
- **The generator** (`src/styles/generator.ts`) is pure and seeded (mulberry32): the same style, key, seed and take always give the same steps. It writes onto a bank's *actual* pads:
  - **Drums** by role (kick, snare, ghost, clap, rim, hat, open hat, ride, crash, tom, perc, shaker), each resolved to the loaded kit's pad by voice name, then by kind — so any kit works, and a role the kit lacks is skipped.
  - **Bass** follows the progression: the root on every bar's downbeat, otherwise root / fifth / octave by the style's odds, and sometimes a walk into the next chord's root.
  - **Chords** play the progression's chords on the style's rhythm (matched to the chord pad with the same notes, else the same root) — every chord change always sounds.
  - **Melody**: a one-bar contour (mostly steps, sometimes leaps, in the style's register) repeated every bar, strong-beat notes snapped to the chord playing, the last bar answering a step higher.
  - All layers of a beat share one progression (from the beat's seed), so they fit.
- **Beat sheet** (the ✦ button on the Sequencer, and a "Blank canvas? Start from a style" prompt on an empty pattern): style cards with a tiny kick/backbeat/hat preview, then
  - **Start a beat**: the style's tempo; the current mood if the style suits it, else the style's first mood; every bank's sound; and all four layers on a fresh pattern of the style's length (`START_PATTERN`). Confirms first if anything is programmed. The sequencer then folds every bank to its used rows.
  - **One layer at a time**: **Add** writes a bank's layer in the chosen style (building the style's sound only if the bank has none suitable — a sound you picked is kept); **New take** rerolls it (`groove.takes`) while the other layers stay put. A layer shorter than the pattern is tiled across it.
- The beat's style, seed and takes are project data (`AppState.groove`). The Phase 1 loop presets were folded into this and removed.
...
interface AppState {
  ...
  groove: { styleId: string; seed: number; takes: Partial<Record<BankKind, number>> } | null; // the style/seed a beat was written in — see Styles
