...
- **Loop Mode and Instrument Mode are mutually exclusive playback selections; Mixer Mode is a non-destructive level-control overlay.** Loop and Instrument change what a pad press means, so selecting either turns the other off. Mixer replaces pads with faders temporarily but keeps the selected instrument underneath, returning to that same playable layout when it closes. All three controls remain always visible in the Pads header (see Layout).
...
- **Every "are you sure?" prompt is a shared `<ConfirmDialog>`**, not an inline block dropped into whatever panel raised it — replacing a pad/instrument/pattern/project, deleting a library sample or instrument, and clearing everything all use it. Portals to `document.body` on its own fixed, centered backdrop, same reasoning as `<Overlay>` above but centered rather than anchored to whatever triggered it, and at a higher z-index, since a confirm is very often raised from *inside* an already-open Overlay (an instrument or loop-preset picker, the pad library picker, Settings) and needs to read as a distinct layer stacked above that sheet.
...
- **Floating record + metronome cluster** (fixed, global on every page except while the pad edit popup is open, bottom-right): press-and-hold the record button to record — the hold itself *is* the recording gesture, release stops it, however long that was. Recording was never tied to one screen or page; a name/waveform-preview/assign-or-discard review appears as a modal overlay regardless of where you started recording from, so it's never blocked by which page happens to be open. A metronome toggle sits beside it. This used to be a four-button cluster (record, metronome, a **Playthrough** toggle, and a **grid-mode** FAB), but the other two have both since moved to the Pads panel header instead (see below) — Playthrough because it's a pad-grid-adjacent concern that deserved a visible, distinct home rather than a small icon easy to miss next to the mic button; the grid-mode FAB because once Loop Mode and Instrument Mode had each earned their own header control, it was left guarding a single remaining menu item (Mixer Mode), which doesn't need a FAB+menu at all — just a plain header toggle alongside the other two modes. Metronome and Record are the only two left here now, both reachable regardless of which pad edit state or page you're in.
- **Pads page (home)**: the pad grid is the hero content, full width, nothing sharing space with it, laid out 3-per-row so each pad is a generous ~100×100px tap target (default pad count 9 fills this to a clean 3×3). The panel header carries five always-visible, grid-wide controls, top-right, next to the "Pads (N)" heading, left to right: a plain round **Instrument Mode button** (just an instrument-key icon, no text label) that opens the "choose an instrument" picker directly when tapped — including while an instrument is already active, leaving the current pads untouched until a replacement is confirmed. Choosing an instrument always confirms before replacing any pad that already has a sound; see Pad Playback Behavior above for the quick-build/auto-cleanup and per-key-icon details; the **Loop Mode switch** — a real switch look (a track with a sliding thumb, not just a colored button) with a loop glyph; a plain **Mixer Mode button** (a bare on/off toggle, unlike Instrument Mode — turning Mixer Mode on has nothing to ask first); then a thin vertical divider; the **Pad Record button** (`PlaythroughToggle` — see Playthrough recording above), styled with the same "record" red the mic button itself uses, always, not just once it's on, so it visually reads as part of the recording family at a glance rather than blending in with the blue/accent mode toggles beside it; and a small round **FX button** opening the whole-grid effects menu (see Pad Playback Behavior above). The divider marks the boundary between "pad-grid mode" controls (Instrument/Loop/Mixer — what tapping a pad does) and the two that aren't modes (Pad Record — what holding the record FAB captures; FX — a one-shot bulk action). An empty pad shows a faint tint of its own color (not a hollow outline) and a "+" hint rather than the word "empty" — reads as an invitation to tap, not just a status label. A filled pad shows a faint waveform silhouette behind its number (from the same peaks data the Library's thumbnails use) — a quick "which sound is this" visual cue, low-opacity so it never competes with the number/badge on top of it. The pad itself is a single undivided tap-to-play/tap-to-loop zone (see Pad Playback Behavior above) showing only live "playing"/"looping"/"muted" visual feedback and, while it holds an instrument key, a small badge with that key's 1-based position *and* an identifying glyph (see Pad Playback Behavior's Instrument mode bullet for what the glyph shows). In Mixer Mode, pads render as `MixerPadFader` tiles instead (a colored fill plus a percentage) — see Pad Playback Behavior above. Tapping a pad selects it and surfaces a summary bar below the grid with four full-sized actions in a plain 2×2 grid, in reading order: **Mute**, **Effects**, **Edit**, **Library**. Mute and Effects — the most-reached-for actions — carry an accent outline even at rest; Edit and Library are quieter, plain-bordered navigational actions, a deliberate flip from an earlier version where Edit/Library were the colored ones and the two most-used buttons were the plainest. Library opens a popup listing every library sample so you can drop one straight onto the selected pad — the reverse direction of the Library page's own "Assign…" action, which starts from a sample and asks which pad. Moving these off the pad face and into generously-sized buttons fixed a real touch-precision problem: a small icon crowded into the corner of an already-small tile was hard to hit reliably.
- **Pad edit popup** (opened via "Edit," a modal overlay over whatever page you were on — not a navigated-to page, and not a nav tab): the per-pad dials (volume/speed/pitch/filter/pan/grit/echo/reverb, most-used first, with a row of quick-start presets across filter/grit/echo/reverb above them) and the trim editor share the popup sheet. The header shows live playing/looping status tags on the left, the "Pad N" heading centered, and a close (✕) button on the right. Below the header, a horizontally-scrollable strip of every pad (a small swatch each, current one highlighted, a pulsing dot on any that's looping) lets you flip between pads without leaving the editor, paired with one full-sized Loop button that always acts on whichever pad the strip currently has selected. Dismissing it — tapping the backdrop or the close button — is instant and unconfirmed: every dial/trim change here dispatches immediately, so there's nothing "unsaved" a popup dismissal could lose (this replaced an earlier full-page version with an edge-swipe-to-go-back gesture and a leave confirmation, both made unnecessary once it became a popup with nothing to protect).
- **Sequencer page**: the 16-step grid gets its own page, meant to be visited once the pads are filled in and it's time to assemble a pattern — not something you're nudging past while trying to do something else. Its header pairs the "Sequencer — {pattern name}" heading with a round **loop-preset menu button** top-right (see Loop presets above) — a compact icon opener rather than its own page, since picking a preset there is really just a fast way to fill in this same grid. Restyled to read as a continuous timeline rather than an isolated checkbox grid: steps within a beat sit close together, each 4-step beat group is set off from the next by a real bar-line border (not just a background tint), and the currently-playing step glows instead of just outlining. Each row shows a small pulsing badge (and a ring around its pad-number circle) whenever that pad is independently looping via the pad-page loop button, so the sequencer's own step pattern is never mistaken for — or silently coexisting unnoticed with — a separate loop already playing. **Tapping a row's color swatch opens the same library picker the Pads page uses**, so you can swap what a row plays without leaving the Sequencer — the reverse-assign flow reused a second time. **"+ Add row"** below the grid grows `visiblePadCount` by one, the same mechanism Settings' pad-count stepper already used. **"Bounce to Pad"** sits beside it — renders the pattern exactly as programmed (mute/trim/effects/mix-level respected, disabled when nothing's been stepped in) down to a single new sample via an offline `OfflineAudioContext` render, reviewed through the same popup a recording uses — completing the pad → beat → sequence → pad loop the whole app is built around. Its review popup deliberately does **not** auto-preview-loop the bounce the instant it opens, unlike a plain mic recording — a bounce (or a Playthrough capture) was very likely built from pads/loops still playing right now, and blasting it back on top of that live mix the moment the popup appears would be exactly the messy overlap a review step shouldn't cause; the play/pause button still works, it's just not pressed automatically (see Recording & the Sample Library below). **"Clear Sequence"** sits beside Bounce — a `btn-ghost-danger` action, disabled when the pattern has no active steps, that clears every step across every pad the pattern tracks (visible or not) after a confirm prompt, distinct from Settings' "Clear All" (which resets the entire project, not just the current pattern). **"Load sequence"** sits beside "Save sequence" — opens a picker of every previously bounced sequence still in the library (see "Loading a saved sequence" under Recording & the Sample Library below), the same load a sample's own Library card offers, reachable here without a trip to Library first. Each row can be removed outright with an **✕** at its left edge (guarded by a confirm dialog, disabled once only one pad remains) — a real splice out of `pads`, not just hiding it, though every *other* row's programmed steps are unaffected since they're keyed by pad id, not array position. Dragging across a run of cells (rather than tapping one) "paints" a fill across all of them in one gesture, in the same on-direction a tap-to-fill already goes; a **"Preview on/off"** toggle beside the existing **"Gate"** button controls whether filling a cell (by tap or by drag) also auditions it once, so a fast multi-cell paint can stay silent instead of firing every intermediate cell. The PlayBar (play/pause, BPM, loop-mode) appears whenever the Sequencer is visible — on a narrow screen that's only the Sequencer page itself; on a wide or landscape combined layout (see below) it's also visible while the Pads tab is selected, since the Sequencer is showing alongside it there too.
...
- **Landscape/ultra-wide layout, fully automatic** (`useIsLandscapeLayout`, a `matchMedia('(min-aspect-ratio: 2/1)')` hook — same `useSyncExternalStore` pattern as `useIsWideScreen`): a landscape phone or an ultra-wide-but-short desktop window gets a different combined arrangement than the plain wide split above — Sequencer stacked on top, Pads below, both stretched to the window's full width — since a side-by-side column pair would otherwise squeeze both down to an unhelpful sliver on a window that's wide but not tall, where the useful direction to spend the extra space is width within each section, not another column. Checked before the plain wide split, so an ultra-wide desktop window (wide by either measure) gets the stack.
- **Library page**: the page is dedicated to the sample library — a focused, occasional-use browsing surface that stays out of the way of the pads. Applying an existing instrument or quick preset to the grid lives on the Pads page (Instrument Mode); applying it also adjusts the visible pad grid to the instrument’s actual key count. Instrument management is deliberately not surfaced in Library. The sample library itself is a **grid of cards**, not a list (`repeat(auto-fill, minmax(150px, 1fr))`) — each card leads with a large waveform (recognizing a sound by its shape is faster than reading its name) above a row of at-a-glance facts: a kind badge (🎤 Recording / 🎹 Note / 🥁 Sequence — see `Sample.kind`), duration, and a simple loudness descriptor (Quiet/Medium/Loud, thresholded off the sample's own peak amplitude — no new audio analysis). Rename, reorder, Assign…, and Delete all still work the same as before, just inside a card instead of a row. **`kind: 'note'` samples (an instrument's generated keys) never appear in this grid at all** — they're real Samples under the hood (so pads can reference them like anything else), but showing all 16 of an instrument's keys as individual cards the moment you build one instrument would flood this page; they are transient backing data for an Instrument Mode performance instead. A quick-built instrument and all 16 of its note samples are removed only when Instrument Mode is explicitly turned off or the instrument is replaced. Loop and Mixer Mode continue to operate on the current pad sounds without discarding them. On explicit cleanup, the pre-instrument pad assignments are restored first.
