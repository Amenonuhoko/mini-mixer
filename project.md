...
- **Loop Mode and Mixer Mode are mutually exclusive; both live on one labeled switch — `PLAY · LOOP · MIX`** (`PadModeSwitch`) — at the top of the pad module. Play turns both off; tapping Mix again leaves it. What the pads *are* (drums, bass notes, chords) is the bank's job, not a mode — the old KEYS mode and its per-performance "auto instrument" were replaced by pad banks (see Pad banks above).
  ...
  - **Mixer mode on**: the bank on screen gets **one volume slider for all its pads** (`Bank.volume`, 0–100, on top of each pad's own `mixLevel` — live in the engine via `setBankVolume`, and in bounces), the **effects for all its pads** (`BankEffectsPanel`: on/off, presets — folded behind one row naming the one in use ("Presets · Clean", or Custom), tap to open the grid —, **Pitch** in semitones for this bank *in the pattern in the grid* (`Pattern.pitch`, ±12; labelled with the song sections that play that pattern — raise it while editing the Verse and the Chorus is untouched; sections sharing a pattern share it; copies, duplicates and endings carry it; −12…+12 with ±12 an octave, any other amount on Bass / Chords / Melody noting that it leaves the song's key; presets leave it alone, Reset zeroes it). The sequencer, song playback and bounces add each pattern's pitch to its own notes; taps, loops and Perform follow the pattern in the grid (`engine.setLivePitch`) — then the four character dials, save preset, reset). A pad's own Pitch dial also counts whole semitones now (stored on the same −100…100 dial, ±100 = ±1200 cents, so older values read as multiples of 3 st) and a reserved space for a real mixer to come. The pads become plain tiles showing each pad's own level (or Muted); tapping one opens that pad's sheet (see Layout). Nothing plays from touching them. The pinned row under the grid swaps Gate / Perform / step-record for a hint.
...
- **Where presets live** — both places, never modal:
  - **Styles live in Make a beat** (top of Seq): each part's row picks its style (dropping that style's layer into the bank), sets sparse ↔ busy intensity, rolls a new take or clears it; the beat's chords sit under the rows with **New** (a new progression every style layer follows). The old Styles drawer, its ✦ button and the bank heads' style chips are gone — one place for styles.
...
- **Transport strip** (top, every page): a beat LED, tempo (hold − / + to run, drag the readout, tap it for the tempo panel — see `TempoControl`) and **Settings** (which now also holds **Playback: Loop / Once**, `LoopModeSetting`).
- **Play's cluster** (in the tab bar; `TransportCluster`): one pill with Play raised dead centre, **metronome** and **master volume** (its slider pops up) on the left, **stop all** on the right.
- **Tab bar** (bottom, thumb zone): **Pads · Seq · [metronome · volume · ▶ Play · stop all] · Song · Library** — Play is the button pressed most, so it sits raised dead center, right under the thumb (`PlayButton`). Recording moved to the Pads page. In a combined layout (wide or landscape) Pads and Seq light up together, since both are showing.
...
- **Make a beat** (top of the Sequencer only; `BeatStarter`; **folds to its title** with the ▴ by its name, remembered across pages): one **row per part** — Drums · Bass · Chords · Melody — whose chip picks it for Generate / Tweak, then its style, new take and clear, and under it two sliders: **Busy** (sparse ↔ busy) and **Keys** (**Kit** for drums: the keys or drums the style itself uses ↔ all of the bank's; `GrooveLayer.range`, 0 = as before). Kit brings in the kit's other drums in a fixed order with their own groove lines (pickups, ghosts, off-beat percussion, a crash on the one, tom runs into the next bar); Keys moves bass and melody notes an octave away, to other chord tones or passing notes — favouring keys not yet played, but never the downbeat root — and lets passing chords in between and just before the changes. Busy sets how many notes; Keys where they land, so both up reaches most of the keyboard. the chords with **New**; and **Generate** (it names the parts when not all are picked, e.g. "Generate Drums + Bass"). Every part picked gets fresh, randomly generated material from the chosen style (**🎲 Surprise me**, the default, picks a random style each press and names it); parts left out stay exactly as they are (`useGroove.regenerateLayers`), and all four — or an empty pattern — is a whole new beat (`startBeat`, which now builds sounds only for the parts it generates). **Options** (folded once there's a beat): style, Simple / Balanced / Complex, 1 / 2 / 4 bars (for a whole beat), **New sounds**, and the **Tweak** row — Sparser, Busier, More syncopated, New take, Add a fill, Opening crash, Short pause, Bass dropout — applied to the picked parts only and auditioned with Keep / Undo. It replaces the old Make a beat + Develop this beat pair (and the Song page's Develop your song).
...
- **Desktop (≥ 900 px wide)**: the sequencer runs the full width across the top; underneath, the pads and Make a beat sit side by side (`.wide-split` grid areas; `Sequencer withBeatStarter={false}` plus a `PatternBeatStarter` of its own). Phones keep Make a beat at the top of Seq. A short, very wide screen (≥ 2:1 and ≤ 560 px tall — a landscape phone) uses the same arrangement edge to edge with the pads taking the extra width (`.landscape-stack`); a tall ultra-wide monitor keeps the desktop layout, which grows to 1760 px on screens ≥ 1800 px wide.
- **App icon**: a 3×3 pad grid in the app's own look — dark pads with cyan edges, a beat lit across them in cyan and the middle (playhead) pad orange, on the void background. `public/favicon.svg` (browser tab, rounded) with a 32 px PNG fallback; `apple-touch-icon.png` (180 px, full-bleed — iOS rounds it) for Add to Home Screen on iPhone; `manifest.webmanifest` (name Beat Maker, standalone) with 192/512 icons and a 512 maskable one whose grid sits inside Android's safe circle.
