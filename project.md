...
- **Bottom PlayBar** (fixed, global on every page): play/pause, BPM, loop-once-vs-continuous. Shapes *how* playback sounds; deliberately kept separate from the top nav's utility actions, which are about getting around and emergency control instead. The metronome used to live here too but moved out — see the record FAB bullet below — since it's independent of play/pause and belongs with the app's other page-agnostic utilities, not the sequencer transport.
...
- **Pad edit page** (reached via "Edit Sound," not a nav tab — a drill-down detail view, not a top-level destination): the per-pad dials (volume/speed/pitch/filter/grit/echo, most-used first) and the trim editor get the entire page. A back button returns to Pads. The header is a 3-column layout — back button left, the "Pad N" heading centered, live playing/looping status tags right — so the heading reads as a page title centered in its own row rather than drifting depending on whether a status tag happens to be present.
