...
- **Three mutually-exclusive global modes govern the pad grid — loop mode, instrument mode, and mixer mode, all off by default.** Turning one on turns the others off (enforced in the reducer): each changes what interacting with a pad means for the grid as a whole, so having more than one on at once would be ambiguous. All three now have their own always-visible control in the Pads panel header (see Layout) — Mixer Mode's used to live behind a menu on a floating grid-mode FAB, but once Loop and Instrument Mode had each earned a dedicated header control, that FAB was left guarding a single remaining option, more UI than a plain toggle needs, so it was removed outright and Mixer Mode joined the other two in the header.
