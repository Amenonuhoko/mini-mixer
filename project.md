...
- **Two mutually-exclusive global modes govern the pad grid — loop mode and instrument mode, both off by default, chosen from one grid-mode button** (see Layout — a single FAB opens a small "Off / Loop Mode / Instrument Mode" menu; the two used to be separate FABs). Turning one on turns the other off (enforced in the reducer): they each change what the grid means as a whole, so having both on at once would be ambiguous.
...
- **Floating record + metronome + grid-mode cluster** (fixed, global on every page except while the pad edit popup is open, bottom-right): press-and-hold the record button to record — the hold itself *is* the recording gesture, release stops it, however long that was. Recording was never tied to one screen or page; a name/waveform-preview/assign-or-discard review appears as a modal overlay regardless of where you started recording from, so it's never blocked by which page happens to be open. A metronome toggle sits beside it, and a single **grid-mode button** (its icon reflects the current mode — off/loop/instrument) opens a small menu to switch between Off, Loop Mode, and Instrument Mode — Loop Mode and Instrument Mode used to be two separate FABs, consolidated into one once a third FAB started visibly crowding whatever page content sat in that corner. Picking Instrument Mode from the menu flows straight into the existing "choose an instrument" picker. The grid-mode button is the one hidden while the pad edit popup is open: that popup has its own pad-specific loop button (see below), so the global one would sit right next to it doing something confusingly different.
- **Pads page (home)**: the pad grid is the hero content, full width, nothing sharing space with it, laid out 3-per-row so each pad is a generous ~100×100px tap target (default pad count 9 fills this to a clean 3×3). An empty pad shows a faint tint of its own color (not a hollow outline) and a "+" hint rather than the word "empty" — reads as an invitation to tap, not just a status label. The pad itself is a single undivided tap-to-play/tap-to-loop zone (see Pad Playback Behavior above) showing only live "playing"/"looping"/"muted" visual feedback and, while it holds an instrument key, a small numbered badge (that key's 1-based position, low to high) — showing the actual key number, not a repeated generic icon, is what makes the grid read as an ordered keyboard once an instrument is applied. Tapping a pad selects it and surfaces a summary bar below the grid with four full-sized actions in a plain 2×2 grid, in reading order: **Mute**, **Effects**, **Edit**, **Library**. Mute and Effects — the most-reached-for actions — carry an accent outline even at rest; Edit and Library are quieter, plain-bordered navigational actions, a deliberate flip from an earlier version where Edit/Library were the colored ones and the two most-used buttons were the plainest. Library opens a popup listing every library sample so you can drop one straight onto the selected pad — the reverse direction of the Library page's own "Assign…" action, which starts from a sample and asks which pad. Moving these off the pad face and into generously-sized buttons fixed a real touch-precision problem: a small icon crowded into the corner of an already-small tile was hard to hit reliably.
