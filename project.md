...
- **Desktop (≥ 900 px wide)**: the sequencer runs the full width across the top; underneath, the pads and Make a beat sit side by side (`.wide-split` grid areas; `Sequencer withBeatStarter={false}` plus a `PatternBeatStarter` of its own). Phones keep Make a beat at the top of Seq; a landscape phone still stacks Seq over Pads.
