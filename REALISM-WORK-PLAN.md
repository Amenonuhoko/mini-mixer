...
- Verified: 201 tests pass, production build passes, only 3 pre-existing Fast Refresh lint warnings. Browser checked Hear, Keep, Undo and restored settings; 412 x 915 layout fits. Offline dense mix peaks at 0.781, RMS 0.183. Mid-attack/sustain/release fallback checks show no abrupt cut (largest adjacent step 0.00181); this does not measure Pixel audio underruns. Retained reproducible manual harness at scripts/audio-phrasing-check.html (copy into app while Vite runs, then open it locally).
- Sustained Organ/Pad/Lead/Velvet Strings default to connected phrasing; voice limiting ignores notes already ended at the scheduled time. Final browser/phone retest remains required on the actual Pixel after deployment.
- Fixed nonexistent Ride Bow Accent and Splash recording filenames exposed during browser checks. Verified all 37 recorded drum references exist in the upstream source tree; these voices now request the recordings instead of falling back to synthesis after a 404.
