...
## 2026-09-12 — Reconciling with a parallel branch: Loop presets and a centered ConfirmDialog ported onto this branch's own architecture

### Context
A separate line of work (`claude/beat-maker-project-review-hrbucr`) had built two features against an older snapshot of this project — a bundled "Loop presets" menu on the Sequencer page (pick a preset, it builds an instrument and programs the pattern), and a shared, professionally-styled centered `ConfirmDialog` replacing every inline `.confirm-overwrite` prompt in the app — without knowing this branch (`codex/remove-library-instruments`, the actual default branch) had continued on its own with ~250 more commits in the meantime: real recorded instrument samples, a 32-pad limit, Gate/one-shot playback modes, master volume, a completely different sequencer model (variable-length patterns, steps that store the exact sample id placed on them rather than a boolean, "sequence traces"), non-destructive Instrument Mode (pads restore via a snapshot instead of being torn down), and `InstrumentLibrary.tsx` kept as the Library's own instrument-management panel rather than removed. The two features were asked to be ported onto *this* branch as the real trunk, not merged blindly — a plain `git merge` produced conflicts in 12 files, several going deeper than text (a genuinely different `Pattern.steps` shape, a different confirm-or-not philosophy in places).

### Decision(s)
**Loop presets, rebuilt against the current step/pad/instrument model rather than replayed as a diff.** The old version assumed `Pattern.steps: Record<string, boolean[]>` and a fixed 16-pad ceiling; this branch's `Pattern.steps: Record<string, Array<string | null>>` (each cell holds the *sample id* placed there, or null) and variable `stepCount` (4–64) meant the whole "how does picking a preset become sequencer data" question had to be re-answered, not just re-typed. New `APPLY_LOOP_PRESET` reducer action (added right after `APPLY_INSTRUMENT_TO_PADS`, whose "grid follows the instrument's key count" resize logic it deliberately mirrors line-for-line) takes a built `Instrument`, its `keySamples`, a target pattern id, and `stepsByPadIndex: Record<number, number[]>` (just the active step indices per pad — no need to pass a sample id per cell, since it's always `instrument.keySampleIds[padIndex]`); in one transaction it resizes `pads`/`visiblePadCount` to the instrument's key count, assigns keys, and replaces the *entire* active pattern (not just the touched rows, since every visible pad's sound is also being replaced) with the preset's cells, growing `stepCount` to at least 16 first if the pattern had been shrunk smaller. `engine/loopPresets.ts` (`LoopPreset`, `LOOP_PRESETS`, `padIndexForHit`, `buildLoopPresetInstrumentKeys`, `loopPresetKeyLabels`) ported with two required adaptations: the default Acoustic Drums kit's real voice names don't match the old fixed 16-voice kit's role names (no plain `'Snare'` or `'Clap'`, for instance) — presets now reference `'Snare Center'` and `'Side Stick'` in their place, still resolved by exact name via `DRUM_KIT_VOICES.find`; and `SynthPatch` now requires a `voice` field selecting a procedural model (piano/bass/lead/pad/pluck/organ/bell/guitar) — the Bass/Melody categories' patches deliberately use `voice: 'pluck'` rather than `'bass'`/`'guitar'`, since either of the latter would make `buildInstrumentKeysFromPreset` try a recorded-sample network fetch first (with a synthesis fallback) before building — fine for a deliberate one-time instrument build, but wrong for something meant to feel instant every time it's picked.

**A loop preset's build is tracked exactly like `InstrumentModeButton`'s own quick presets** — the same `Transport.autoInstrumentId`/`autoInstrumentPadSnapshot` mechanism, via a new shared `captureAutoInstrumentPadSnapshot` helper (`utils/autoInstrumentSnapshot.ts`, factored out of `InstrumentModeButton.tsx`'s own inline version once a second caller needed it). This means replacing or removing a loop preset's instrument restores the pads it covered non-destructively, for free — this branch's own `REMOVE_INSTRUMENT` case already retains any key sample still referenced by a pattern cell, so the programmed steps keep playing correctly even after their originating "temporary" instrument is gone.

**A real gap found and fixed along the way**: this branch has no code path that cleans up a temporary/"auto" instrument when Instrument Mode turns off via switching to Loop Mode (`SET_PAD_LOOP_MODE_ENABLED`) — only `InstrumentModeButton`'s own "pick a new preset" flow ever calls `REMOVE_INSTRUMENT` on the old one. Restored the `Shell`-level `useEffect` (from the branch this work ported from) that watches `padInstrumentModeEnabled` and fires the cleanup whenever it goes false for *any* reason — needed for our own Loop presets to not leak generated samples on a mode switch, and it closes the same gap for `InstrumentModeButton`'s existing quick presets too, which had the identical exposure already.

**`ConfirmDialog` ported to every confirm site that still exists on this branch**, adapting to two real differences from the source: this branch had *more* sites than expected (`InstrumentLibrary.tsx`'s own delete-instrument confirm, and a `Nav.tsx` "Load project" flow using a native `window.confirm`/`window.alert` — Nav duplicates Settings' own Save/Load, apparently added independently on this branch; both got the same treatment, `Nav.tsx`'s converted from an imperative confirm into a `pendingLoad` state + dialog, same shape Settings' own already used), and *fewer* in two specific spots this branch had deliberately removed for good reasons of its own: `InstrumentModeButton` no longer confirms before replacing an instrument (replacement is non-destructive now — the old pads are snapshotted and restored, not destroyed) and `PadEffectsMenuButton` applies a preset immediately with no confirmation at all (a deliberate simplification, unrelated to the confirm-dialog work). Neither reintroduced a confirm dialog — that would fight a design decision already made here for reasons that hold regardless of styling. Everywhere a confirm remained, though, it's the same centered `<ConfirmDialog>`: `Library`, `InstrumentLibrary`, `PadAssignPrompt`, `PadLibraryPicker`, `RecordingReview`, `SettingsPanel` (both its own confirms), `Nav`, and the Sequencer's own Clear Sequence confirm — which had its own bespoke anchored-popover CSS (`.sequencer-clear-control`) on this branch, since removed along with the rest of `.confirm-overwrite`'s styling.

### Alternatives considered
- **A literal `git merge`/rebase, resolving conflicts hunk by hunk** — tried first (on a scratch branch, nothing pushed) specifically to measure the real scope before deciding how to proceed. Rejected as the actual integration strategy once it produced conflicts in files whose underlying data model (`Pattern.steps`) had fundamentally changed shape — a mechanical merge would have produced code that type-checked in neither branch's mental model. Manually re-deriving the two features against this branch's actual current architecture, instead of trying to reconcile two versions of a diff, was the only way to end up with something that was actually correct here.
- **Reusing `voice: 'bass'`/`'guitar'` for the Loop presets' Bass category**, to get the more characterful procedural bass/guitar models — rejected once it became clear `buildInstrumentKeysFromPreset` treats those two voices as recorded-sample candidates first: a loop preset is meant to be instant, repeatable, mix-and-match use, and a network round-trip (even a fast-failing, gracefully-falling-back one) on every pick works against that.
- **Reinstating the "New from a recording…" build flow inside `InstrumentModeButton`** (present on the other branch, restored there after being removed) — turned out unnecessary: `InstrumentLibrary.tsx` on this branch already offers exactly this, just from a different surface (the Library page's own management panel rather than the pad-grid picker), so nothing needed porting here at all.

### Reasoning
Both ported features are additions to areas of the codebase that had moved substantially since they were written — treating the port as "make the same UI outcome true here" rather than "apply the same diff" is what let each one come out consistent with decisions this branch had already made (non-destructive instrument replacement, sample-per-step sequencer data, no unnecessary network dependency) instead of quietly reintroducing behavior this branch had deliberately moved away from.

### Outcome
Full verification: `tsc -b` (clean), `oxlint` (same three pre-existing `only-export-components` warnings), `vitest run` (91/91 — 89 pre-existing plus 2 new for `APPLY_LOOP_PRESET`; also fixed one pre-existing failing assertion in `applying an instrument lays its keys across the visible pads in order, resetting trim`, stale relative to this branch's own "grid follows the instrument's key count" resize behavior and failing already on a clean checkout of this branch before any of this work started), `vite build` clean. Playwright pass at 390×844: confirmed the loop-preset menu shows 15 cards across the three categories; applied a fully-synthetic Bass/Melody preset end-to-end (correct step count, `ConfirmDialog` centered and layered above the still-open picker when replacing an existing pattern, Instrument Mode turned on); confirmed switching to Loop Mode afterward correctly turned Instrument Mode off (the restored cleanup `useEffect` firing as intended). The Drums-category path (the default Acoustic Drums kit, which tries CC0 recorded samples over the network before falling back to synthesis) could not be exercised to full completion in this sandbox specifically, since its egress proxy blocks the sample host outright rather than failing fast — confirmed instead that `LOOP_PRESETS`' voice-name references all resolve correctly (a bad name throws at module load, and the app loaded and rendered all 15 preset previews without error) and via two new reducer-level tests that don't depend on real audio rendering; the code path itself is unmodified, already-shipped logic (`buildDrumKitKeys`), exercised identically by this branch's own `InstrumentModeButton`/`InstrumentLibrary` Drum Kit picks. `project.md` updated: a new "Loop presets" Core Features subsection, a `ConfirmDialog` bullet in Layout beside the existing `Overlay` one, and the Sequencer-page bullet mentioning the new header menu button.

### Open questions / carried forward
- `buildDrumKitKeys`/`recordedVoice`/`decodeRemoteAudio`'s recorded-sample fetches have no timeout — a hard network block (rather than a fast failure) can leave a picker's "Building…" state open far longer than a user would tolerate. Pre-existing on this branch, unrelated to this session's two ported features, and not something this session's scope covered fixing.
- No other items are currently queued.

---

## 2026-09-12 — A batch of Sequencer/Effects/Layout requests: instrument-tied effects, landscape layout, row removal, drag-paint, and a real sequence-load

### Context
Thirteen separate requests arrived in one message, spanning the effects UI, the whole-grid effects preset menu, a landscape/ultra-wide layout, and several Sequencer-row mechanics. Handled as one batch since several touch the same files (`Sequencer.tsx`, the reducer, `types.ts`).

### Decision(s)
**Effects editor always stops its loop on close.** `PadEditPage` now keeps a ref (`stopOnCloseRef`) synced every render via its own effect (not written during render — see Reasoning) with the current engine/pad id/looping flag, and an unmount-only effect reads that ref in its cleanup to stop the pad if it was looping — covering every way the popup closes (✕, backdrop, switching to another pad's edit view), not just one handler.

**The effects on/off toggle merged into the effects panel's own header** as a shared `EffectsSwitch` component (a real switch look, matching the pattern `Layout` already uses for Loop Mode) — both `PadEditPage`'s per-pad panel and `PadEffectsMenuButton`'s whole-grid popup now show the same unambiguous on/off control next to the heading, replacing a plain backward-labeled button in each.

**Whole-grid effects now support live dial customization, tied to the instrument currently laid across the pads.** `Instrument` gained an optional `effectsPreset` (Filter/Grit/Echo/Reverb, the same four-dial scope the existing presets already used) and `Transport` gained `currentInstrumentId`, set whenever `APPLY_INSTRUMENT_TO_PADS`/`APPLY_LOOP_PRESET` lays an instrument's keys across the grid and cleared when that instrument is removed. `PadEffectsMenuButton` grew four live sliders below its preset grid; dragging one dispatches `SET_ALL_PADS_EFFECT` (new — parallels the existing per-pad `SET_PAD_EFFECT` but scoped to every visible pad), and every action that touches all-pads character effects (`SET_ALL_PADS_EFFECT`, `APPLY_EFFECT_PRESET_TO_ALL_PADS`, `RESET_ALL_PADS_EFFECTS`) now also writes the result back onto `currentInstrumentId`'s own `effectsPreset` via a shared `syncCurrentInstrumentEffectsPreset` reducer helper. Re-applying that instrument later (switching away and back) always re-lays its saved combo — or neutral, if it never had one — so one instrument's dialed-in character can never leak onto a different instrument sharing the same pad slots. Folding the sync into the reducer's own action handlers (rather than requiring `PadEffectsMenuButton` to dispatch a second action) means no future caller of these actions can forget to keep the two in sync.

**A landscape/ultra-wide layout, fully automatic by aspect ratio.** New `useIsLandscapeLayout` hook (`useSyncExternalStore` + `matchMedia('(min-aspect-ratio: 2/1)')`, the same pattern `useIsWideScreen` already used for its own breakpoint) drives a third page arrangement in `App.tsx`'s `CurrentPage`: Sequencer stacked above Pads, both stretched to the window's full width, checked *before* the existing wide-split so an ultra-wide-but-short desktop window gets the width-maximizing stack rather than being squeezed into two narrow side-by-side columns. `Shell()`'s `combinedView` (drives the play bar and swipe-gesture suppression) now covers either arrangement.

**Sequencer rows can be removed outright — a real splice, not a hide** — via a new `✕` button at the left of each row (a new `.sequencer-row-fixed` sticky wrapper keeps it and the row label aligned with the header row across horizontal scroll), guarded by a `ConfirmDialog` so a slip of the thumb can't silently drop a row's data, and disabled once only one pad remains. New `REMOVE_PAD` reducer action splices the pad out of `state.pads` (decrementing `visiblePadCount` if it was visible), deletes that pad's own `steps`/`traceSteps` entries from every pattern, and clears `currentInstrumentId` — every *other* row is untouched by construction, since `Pattern.steps`/`traceSteps` are keyed by stable pad id, not array index, so splicing the removed pad out of `pads` can never shift which row another pad's data belongs to. A pad's own color/identity stays fixed to the pad object itself through a reorder — nothing about it re-derives from its new position.

**Switching to a smaller instrument, and adding/removing sequencer steps, were already non-destructive by construction** and needed no new code: `APPLY_INSTRUMENT_TO_PADS`/`APPLY_LOOP_PRESET` only ever change `visiblePadCount` (display-only) or grow `pads`, never splice it — `REMOVE_PAD` above is the only action that actually deletes a pad's row data, and it's a distinct, deliberate, confirmed user action. `ADD_PATTERN_STEPS`/`REMOVE_PATTERN_STEPS` already preserved every pad's existing cells while resizing the array. Both had reducer-test coverage already (`shrinking pad count hides pads without discarding their data`, `adds four sequencer cells to the right without changing existing steps`) confirming this holds.

**Dragging across sequencer cells now "slides" a fill across multiple beats**, and filling a cell can be previewed or silenced. Each `SequencerRow` tracks a drag gesture via `setPointerCapture` on the origin cell plus `document.elementFromPoint(event.clientX, event.clientY)` inside `pointermove` (capture redirects the event object to the origin element, so the sibling actually under the finger has to be found by document position, not by which element received the event) — a drag only ever fills cells on, matching the direction a plain tap-to-fill already goes, so painting across a mix of on/off cells stays predictable. A new "Preview on/off" toggle beside the existing "Gate" button controls whether filling a step (by tap or by drag) also auditions it once — off by default expectations elsewhere in the app favor sound-on-input, but a fast multi-cell paint is exactly the case where hearing every intermediate cell fire is unwanted noise.

**Loading a previously bounced sequence is now a real load, not a read-only visual trace.** `SequenceTrace.rows` changed shape from `boolean[][]` to `Array<Array<string | null>>` — the exact sample id placed in each cell at bounce time, the same shape a `Pattern`'s own `steps` already uses — so a load has enough information to restore real, playable steps, not just "something was here." The `LOAD_SEQUENCE_TRACE` reducer case (shared cell-resolution logic factored into `utils/sequenceTraceLoad.ts`, `resolveSequenceTraceCell`) now writes each cell's real sample id straight into the target pattern's `steps` wherever that sample still exists in the library, and falls back to a ghost `traceSteps` marker (unplayable, visual-only, using the same visual language the old hide/restore trace flow already had) only for cells whose sample has since been deleted — `traceSteps`/`traceSource` are left `null` entirely when nothing was missing, so a fully-intact load shows no ghost overlay at all. This is a full pattern *replacement*, not a merge: any of the active pattern's prior content outside what the trace covers is cleared, matching what "load a saved sequence" should mean. A shared `LoadSequenceButton` component (confirm-first, since it's destructive to the active pattern) shows the step count and exactly how many hits will and won't come back — e.g. "Loaded 16 steps, 2 hits restored," or "...12 of 16 hits restored — 4 samples no longer in the library" — *before* committing, in the confirmation prompt itself, rather than a toast after the fact (which would have needed cross-page state to survive the same dispatch's navigation to the Sequencer). Used both from the Library's per-sample action (renamed "Trace" → "Load") and a new "Load sequence" button/picker on the Sequencer page itself, so reaching it never requires a trip to Library first.

### Alternatives considered
- **A separate saved-patterns library, distinct from the sample library** — considered for "load a previously saved sequence," rejected once it became clear the existing bounce → `kind: 'sequence'` sample → `sequenceTrace` mechanism already *is* a saved pattern, just underexploited (its only consumer treated the trace as a read-only ghost overlay). Upgrading that existing path to a real load reuses a mechanism users already reach for ("Save sequence") rather than teaching a second, parallel "save" concept.
- **A post-load toast for the step-count summary** — rejected in favor of showing it in the pre-load confirmation dialog instead, since navigating to the Sequencer as part of the same action would unmount whatever component owned toast state (Library's "Load" button lives on a different page than where the load becomes visible), and showing the count *before* committing is more useful anyway — it's the information the user needs to decide whether to load at all, e.g. when several referenced samples have since been deleted.
- **Writing `stopOnCloseRef.current = {...}` directly in `PadEditPage`'s render body**, matching an existing pattern used elsewhere in the codebase for "always read the latest value in an unmount cleanup" — worked, but tripped `oxlint`'s `react(refs)` rule (ref access during render). Split into two effects instead: one with real dependencies that keeps the ref current after every render, one empty-dependency effect whose only job is the unmount cleanup — same behavior, no lint warning, and arguably clearer about which effect is doing what.

### Reasoning
Several of these land in the same place for the same reason: keying data by stable id (pad id in `Pattern.steps`/`traceSteps`, sample id in a sequence's `rows`) rather than by position is what makes destructive-looking operations (splicing a pad out of the grid, loading a saved pattern back in) safe to implement as plain array/object rewrites instead of needing bespoke reindexing logic. The instrument-tied-effects sync and the trace-cell-resolution logic both got pulled into shared helpers used by every caller (reducer action handlers, `LoadSequenceButton`) specifically so correctness doesn't depend on every future call site remembering a second step.

### Outcome
Full verification: `tsc -b` (clean), `oxlint` (down to the same three pre-existing `only-export-components` warnings once the `PadEditPage` ref-during-render fix landed), `vitest run` (93/93 — 91 pre-existing plus 2 new covering `LOAD_SEQUENCE_TRACE`'s real-restore behavior, one with a missing sample forcing the ghost-marker fallback and one confirming no ghost trace is written when nothing is missing), `vite build` clean. Playwright pass exercising the new load-sequence flow end-to-end (fake mic device, real `MediaRecorder` capture): assigned a recording to a pad, programmed two steps, bounced, cleared the pattern, opened the Sequencer's new "Load sequence" picker, confirmed the dialog read "Loaded 16 steps, 2 hits restored," loaded it, and confirmed exactly the same two cells came back on — matching the pre-bounce pattern exactly.

### Open questions / carried forward
- The recorded-sample fetch timeout gap noted in the previous entry is still open and still out of this session's scope.
- No other items are currently queued.

---

## 2026-09-12 — The whole-grid FX popup follows the pad you're playing on mobile

### Context
The whole-grid effects popup (`PadEffectsMenuButton`) was a small popover anchored under its own header "FX" button — fine on a wide desktop layout where the whole pad grid is visible at a glance, but on mobile the pad grid can run much taller than the header. Asked for the popup to instead be a free-floating menu that follows wherever pads are actually being played.

### Decision(s)
**On mobile, the open panel repositions to float next to whichever pad was most recently tapped, chosen automatically rather than user-draggable.** `PadGrid` already tracks a `selectedPadId` (every pad tap — filled or empty — calls `onSelect`), so that same value is threaded down as a new `followPadId` prop; both `PadButton` and `MixerPadFader` got a `data-pad-id={pad.id}` attribute so the panel can find a pad's live on-screen position without extra ref-plumbing (the same "just query for it" approach the Sequencer's drag-paint already uses via `document.elementFromPoint`). A new `computeAnchoredPosition(anchorRect, panelSize, align)` picks below-or-above (whichever actually fits the viewport, flipping when clipped) and centers horizontally on the pad (`align: 'center'`) — clamped so the panel can never render partially off-screen. `useIsWideScreen` gates the behavior: `floating = open && !isWide && followPadId !== null`, so a wide desktop layout keeps the plain "anchored under the button" popover, unchanged, since reachability was never the problem there.

**Discovered along the way: the panel needed to be portaled to `document.body`, which meant repositioning the desktop-anchored case too, not just the new mobile one.** The panel was rendered as a plain DOM descendant of `.pad-grid` → `.app-shell`, positioned via CSS (`position: absolute`, anchored to a `position: relative` wrapper). Making it `position: fixed` and letting it fly around the viewport ran straight into a bug this project had already hit and fixed once before (see the "Wide-screen layout" reconciliation entries) but that this specific popup had never been ported to: `.app-shell` is `position: fixed`, which unconditionally opens its own stacking context per spec — a `position: fixed` *descendant* of it has its z-index compared only within that context, not against `.top-nav` (a `.app-shell` sibling with its own explicit `z-index: 10`), so `.top-nav` visually and interactionally sat on top of the popup regardless of the popup's own `z-index: 20`. Caught immediately by a Playwright pass: clicking the popup's own close button silently failed with "element intercepts pointer events" from `<nav class="top-nav">`, the exact same failure signature the earlier `<Overlay>` fix's own commit message documented. Fixed by portaling the panel to `document.body` via `createPortal`, exactly like every other popup in the app — which meant the CSS-relative anchoring the desktop case relied on no longer worked either (a portaled element has no positioned ancestor to anchor against), so `computeAnchoredPosition` grew an `align: 'left'` mode and now drives *both* cases' positions from JS, not just the new mobile one.

**A guaranteed close button, only in floating mode.** Following the last-played pad can genuinely land the panel on top of its own toggle button (a pad played near the header, or a tall panel that had to flip upward) — the anchored desktop case can't hit this since the button always sits in the same visible header spot. Added a small ✕ inside the panel's heading, rendered only when `floating` is true, so there's always a reachable way to dismiss it regardless of where auto-positioning put it.

### Alternatives considered
- **User-draggable panel** (drag it once, it remembers where you left it) — the option offered alongside auto-follow; not chosen. Auto-follow needs no extra gesture and keeps the panel where your hand already is as you move around the grid, which is the more literal reading of "follows where the pads are being played."
- **Closing the panel automatically on scroll/resize instead of repositioning** — rejected; the whole point of a live, playable floating panel is that it survives ordinary interaction (scrolling the grid, rotating the device) rather than making you reopen it constantly.
- **A full-screen dimmed backdrop (reusing `<Overlay>` wholesale)** — rejected; the popup is meant to stay usable *while* pads underneath are still being tapped (that's what makes "follow the pad you're playing" meaningful at all), so a modal backdrop blocking taps elsewhere on the grid would work against the feature's own purpose. It got its own lightweight `createPortal` instead of `<Overlay>`'s backdrop+sheet wrapper.

### Reasoning
The stacking-context bug wasn't a new mistake — it's the same one `<Overlay>` was built to prevent — but this specific popup had grown by extension of the header controls rather than as a new "open a popup" call site, so it never went through that established pattern. Needing this feature is what surfaced the gap between "works fine anchored under a normal-flow button" and "actually needs to `position: fixed` fly around the viewport," which is exactly the condition the earlier fix's own reasoning describes.

### Outcome
Full verification: `tsc -b` (clean), `oxlint` (same three pre-existing `only-export-components` warnings — an initial version tripped `react(set-state-in-effect)` on a trivial "reset to null while not floating" branch, removed since the render already ignores a stale position whenever not floating), `vitest run` (93/93, no reducer-level changes this round). Playwright passes at 390×844 (mobile) and 1100×800 (desktop): confirmed the panel measurably repositions between a bottom-right pad and a top-left pad as `followPadId` changes (screenshots and bounding-box math both checked), confirmed the ✕ close button is reachable even when the panel is pinned to the very top of a short viewport (the exact case that first exposed the stacking bug), and confirmed the desktop-anchored case still sits left-aligned directly under the button and closes via the header toggle exactly as before.
...
## 2026-09-26 — Pitch belongs to the pattern, not the pads

The user: pitching the verse shouldn't change the chorus. The bank Pitch had written every pad's pitch effect, which is global. It now lives on the pattern (`Pattern.pitch`, semitones per bank; `SET_PATTERN_PITCH`), like Phrasing. Playback adds it per pattern: `triggerStep(…, patternCents)` from the pattern actually playing (so a song moves between pitches section by section), bounces add it per hit, and anything played live — taps, loops (retuned on change), Perform — follows the pattern in the grid through `engine.setLivePitch`. The pad's own Pitch dial is unchanged and adds on top. The control is labelled with the sections that use the pattern ("Pitch · Verse"), since that's how the song names it; sections that share a pattern share its pitch. Saved and loaded with the pattern; copied by Duplicate, Copy from, New pattern (copy), endings and Make unique.
